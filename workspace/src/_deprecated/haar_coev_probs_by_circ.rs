...
fn main() {
    ...
    if !circuit_dir.join(Instance::Meas.fmt(circ, None, false)).is_file() {
        panic!("missing measurement locations file for circuit {}", circ);
    }
    println!("running circuit {}", circ);
    ...
    let meas: Elements<Elements<MeasSeq>> = Elements::load(
        circuit_dir.join(Instance::Meas.fmt(circ, None, false))).unwrap();
        ...
        nd::Zip::from(traj_p.outer_iter_mut())
            ...
            .par_for_each(|mut traj_pr, mut prob_pr| {
                let Trajectory { traj, probs } =
                    sample_trajectory((unis.as_ref(), meas_p.as_ref()));
                traj.into_iter()
    ...
    let fname = Instance::Output.fmt(circ, Some(&task_id), true);
    write_npz!(
        outdir.join(fname),
        arrays: {
