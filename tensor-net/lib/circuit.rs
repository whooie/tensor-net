//! Abstractions for layer-based generation of circuits on qubits.
...
pub enum CircuitError {
    ...
    /// Returned when attempting to deserialize malformed data attempting to
    /// deserialize to the wrong data type.
...
///
/// **Note:** matrices *must* be either 2x2 or 4x4, and will be *assumed* to be
/// unitary.
...
impl Uni {
    ...
    pub fn make_mat(&mut self) -> (usize, &nd::Array2<C64>) {
        match self {
            Self::Gate(g) => {
    ...
    pub fn make_mat_rng<R>(&mut self, rng: &mut R) -> (usize, &nd::Array2<C64>)
    where R: Rng + ?Sized
    {
        match self {
            Self::Gate(g) => {
    ...
    pub fn mat(&self) -> Option<(usize, &nd::Array2<C64>)> {
        match self {
            Self::Gate(_) => None,
    ...
    /// unitary.
    pub unsafe fn mat_mut(&mut self) -> Option<(usize, &mut nd::Array2<C64>)> {
        match self {
            Self::Gate(_) => None,
            Self::Mat(k, mat) => Some((*k, mat)),
        }
...
impl From<(usize, nd::Array2<C64>)> for Uni {
    fn from(uni: (usize, nd::Array2<C64>)) -> Self { Self::Mat(uni.0, uni.1) }
}

impl From<(nd::Array2<C64>, usize)> for Uni {
    fn from(uni: (nd::Array2<C64>, usize)) -> Self { Self::Mat(uni.1, uni.0) }
}

impl<'a> From<(usize, Cow<'a, nd::Array2<C64>>)> for Uni {
    fn from(uni: (usize, Cow<'a, nd::Array2<C64>>)) -> Self {
        Self::Mat(uni.0, uni.1.into_owned())
    }
}

impl<'a> From<(Cow<'a, nd::Array2<C64>>, usize)> for Uni {
    fn from(uni: (Cow<'a, nd::Array2<C64>>, usize)) -> Self {
...
impl From<(usize, nd::Array2<C64>)> for Op {
    fn from(uni: (usize, nd::Array2<C64>)) -> Self { Self::Uni(uni.into()) }
}

impl From<(nd::Array2<C64>, usize)> for Op {
    fn from(uni: (nd::Array2<C64>, usize)) -> Self { Self::Uni(uni.into()) }
}
...
/// Load a data from a file using [`serde_cbor`].
