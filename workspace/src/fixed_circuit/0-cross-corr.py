...
def doit(infile: Path):
    ...
    corrdiff = np.nan * np.zeros((runs, *pairdiff.shape), dtype=np.float64)
    pairx = np.zeros((size, size), dtype=np.uint32)
    corrx = np.nan * np.zeros((runs, *pairx.shape), dtype=np.float64)
    pairt = np.zeros((depth, depth), dtype=np.uint32)
    corrt = np.nan * np.zeros((runs, *pairt.shape), dtype=np.float64)
    for ((t0, x0), (t1, x1)) in pairs:
        ...
        if pairdiff[dt, dx] > 0:
            corrdiff[:, dt, dx] = (
                pairdiff[dt, dx] * corrdiff[:, dt, dx]
                + outcomes[:, t0, x0] * outcomes[:, t1, x1]
            ) / (pairdiff[dt, dx] + 1)
        else:
            corrdiff[:, dt, dx] = (
                outcomes[:, t0, x0] * outcomes[:, t1, x1]
        ...
        if abs(dt) == dt0 and dx != 0:
            if pairx[x0, x1] > 0:
                corrx[:, x0, x1] = (
                    pairx[x0, x1] * corrx[:, x0, x1]
                    + outcomes[:, t0, x0] * outcomes[:, t1, x1]
                ) / (pairx[x0, x1] + 1)
            else:
                corrx[:, x0, x1] = (
        ...
        if abs(dx) == dx0 and dt != 0:
            if pairt[t0, t1] > 0:
                corrt[:, t0, t1] = (
                    pairt[t0, t1] * corrt[:, t0, t1]
                    + outcomes[:, t0, x0] * outcomes[:, t1, x1]
                ) / (pairt[t0, t1] + 1)
            else:
                corrt[:, t0, t1] = (
    ...
    corrdiff = (
        np.roll(np.roll(corrdiff, -size, axis=2), -depth, axis=1)
        .mean(axis=0)
    )
    pairdiff = (
    ...
    (
        ...
        .set_xlabel("$x_1$")
        .set_ylabel("$x_0$")
        .set_clabel(f"$\\langle Z(x_0) Z(x_1) \\rangle_{{|\\Delta t| = {dt0}}}$")
        [4]
        .imshow(
            pairx,
            origin="lower",
        ...
        .set_xlabel("$x_1$")
        .set_ylabel("$x_0$")
        .set_clabel(f"$\\#(x_0, x_1)_{{|\\Delta t| = {dt0}}}$")
    )

    corrt = corrt.mean(axis=0)
    ...
    (
        ...
        .set_xlabel("$t_1$")
        .set_ylabel("$t_0$")
        .set_clabel(f"$\\langle Z(t_0) Z(t_1) \\rangle_{{|\\Delta x| = {dx0}}}$")
        [6]
        .imshow(
            pairt,
            origin="lower",
        ...
        .set_xlabel("$t_1$")
        .set_ylabel("$t_0$")
        .set_clabel(f"$\\#(t_0, t_1)_{{|\\Delta x| = {dx0}}}$")
    )
