...
topography \
    = cm.LinearSegmentedColormap.from_list("topography", color_scales["topography"])
...
def figure3D(*fig_args, **fig_kwargs) -> tuple[pp.Figure, p3.Axes3D]:
    """
    Shortcut to creating a figure with a single, empty set of 3D axes.

    All arguments are passed to `pyplot.figure`.
    """
    fig = pp.figure(*fig_args, **fig_kwargs)
    ax = p3.Axes3D(fig)
    return fig, ax

def set_font(path, name: str):
    """
    Set the default font family, provided a path to a suitable font file and a
    name for the font, via `pyplot.rcParams["font.family"]`.
    """
...
def set_fontsize(s: int | str):
    """
    Set the default font size via `pyplot.rcParams["font.size"]`.
    """
    pp.rcParams["font.size"] = s

def use_tex(u: bool=True):
    """
    Turn on TeX fonts via `pyplot.rcParams["text.usetex"]`.

    If `u == True`, select "Computer Modern" fonts, otherwise "DejaVu".
    """
...
def grid(onoff: bool=True, axes=None):
    """
    Set a custom grid format. If `axes` is not `None`, set it for that
    particular set of axes, otherwise the default/current axes.
    """
    if axes:
        axes.minorticks_on()
        if onoff:
...
def set_color_cycle(c):
    """
    Set the default color cycle for line plots via
